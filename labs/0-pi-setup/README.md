...
your r/pi (Model 1 A+ or Model Zero) and ARM toolchain works.
 ...
 - This writeup assumes the use of Parthiv's breakout board rather than
   a standalone tty-usb device --- If you use the latter, the 
   [2023 240LX writeup](https://github.com/dddrrreee/cs240lx-23spr) 
...
3.  Copy all the files from the `firmware` directory at the top level of 
    the class repo to the SD card. (I.e., at `cs240lx-26spr/firmware`.)
...
(in `cs240lx-26spr/bin`) to run the precompiled hello world program
...
------------------------------------------------------------------------
## 5. Set your `CS240LX_2026_PATH`

Checkoff:
   - Typing:

            cd $CS240LX_2026_PATH/labs/0-pi-setup
...
`CS240LX_2026_PATH` to contain where you have your class repository.
...
E.g., for me, since I'm running `tcsh` I edit my `.tcshrc` file and add:
    ...
    setenv CS240LX_2026_PATH /home/engler/class/cs240lx-26spr/

To the end of it.  If you use `bash` you'll do an:

    export CS240LX_2026_PATH=<path to your repo>/cs240lx-26spr/
...
If this doesn't work:
  ...
  2. You mispelled `CS240LX_2026_PATH` or gave it the wrong path.
