...
enum { MAX_SECS = 156 };  
static uint32_t sections[MAX_SECS] = {0};


static int seg_is_alloced(uint32_t pa) {
    uint32_t s = pa >> 20;  // get 1MB segment number
    if(s >= MAX_SECS)
        panic("allocating illegal physical seg: %x [sec=%d]\n", pa, s);
    return sections[s];
}
...
static int seg_alloc(uint32_t pa) {
    ...
    if(s >= MAX_SECS)
        panic("allocating illegal physical seg: %x [sec=%d]\n", pa, s);
    if(sections[s])
        panic("physical seg already allocated: %x [sec=%d]\n", pa, s);
    return sections[s] = 1;
}
...
uint32_t seg_find_free(void) {
    uint32_t s = seg_avail >> 20;
...
// simplistic pinned identity map.
static inline uint32_t ident_pin_mem(uint32_t idx, uint32_t va, pin_t pin) {
    assert(idx < MAX_PIN);
    assert(va % MB(1) == 0);
    
    // you can change this to PAGE_16MB, but need to 
    // change the code to mark all 16MB as allocated.
    assert(pin.pagesize == PAGE_1MB);

    if(!seg_alloc(va))
        panic("already allocated %x\n", va);
    pin_mmu_sec(idx, va, va, pin);
    return va;
}

static inline uint32_t ident_pin_dev(uint32_t idx, uint32_t va, pin_t pin) {
    assert(idx < MAX_PIN);
    if(va < SEG_BCM_0 || va > SEG_BCM_2)
        panic("illegal bcm addr %x\n", va);
    pin_mmu_sec(idx, va, va, pin);
    return va;
}
...
int vm_map_everything(uint32_t mb) {
    ...
    unsigned idx = 0;
    ident_pin_mem(idx++, SEG_CODE, kern);

    // we could mess with the alignment to give the
    // heap more memory.
    ident_pin_mem(idx++, SEG_HEAP,        heap);
    ident_pin_mem(idx++, SEG_STACK,       kern);
    ident_pin_mem(idx++, SEG_INT_STACK,   kern);
    ident_pin_dev(idx++, SEG_BCM_0,       dev);
...
uint32_t vm_map_seg(uint32_t addr, uint32_t dom, uint32_t perm) {
    if(pin_idx >= MAX_PIN)
    ...
    ident_pin_mem(pin_idx++, addr, attr);
