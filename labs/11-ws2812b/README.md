...
-------------------------------------------------------------------------
### Part 0: Implement timing routines (`0-timing`)

For this you should work through the different programs, building out
`gpio-raw.h`.  
  - Expected error should be less than 10 cycles for each
    operation when cached and less than 20 uncached.  (These are 
    pretty generous values: smaller is better.)

The previous two labs should help you figure this out pretty fast.

Extension: Measure that these routines actually have the measured 
timings by:  
  - Using another pi to build a logic analyzer (or use 340lx's first
    lab);
  - Using DMA or SMI
  - It's a fun puzzle.
...
NOTE: 
  - ***After part 0 (which is new this year), this part should be
    a formality.  You should be able to just define the enums in
    `WS2812B.h` and run this and pass.***
...
The workflow: 
  1. `1-code/0-timing-check.c` has 6 parts.  
